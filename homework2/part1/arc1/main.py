...
with tf.Session() as session:
    ...
    for epoch in range(m.epochs):
        ...
        test_accuracy = sum(sum(conf_mxs).diagonal()) / test_im_num
        print('TEST ACCURACY: ' + str(test_accuracy))
        lower, upper = util.conf_interval_calc(1-test_accuracy, test_im_num)
        print('95% CI ON TEST ERROR [' + str(lower) + ',' + str(upper) + ']')
